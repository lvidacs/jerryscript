                        ...
                        ecma_string_t *name_p) /** < property name */
{
  ...
  ecma_completion_value_t completion_value = ecma_op_object_get (holder_p, name_p);

  if (!ecma_is_completion_value_normal (completion_value))
  {
    return completion_value;
  }

  ecma_value_t value = ecma_copy_value (ecma_get_completion_value_value (completion_value), true);
  ...
  if (ecma_is_value_object (value))
  {
    ...
    ecma_property_t *properties_p = ecma_get_property_list (object_p);
    ...
    if (no_properties > 0)
    {
      ...
      for (property_index = 0; property_index < no_properties; property_index++)
      {
        ...
        if (ecma_is_value_undefined (value))
        {
          ecma_completion_value_t comp_val = ecma_op_general_object_delete (object_p,
                                                                            property_names_p[property_index],
                                                                            false);
          ecma_free_completion_value (comp_val);
        }
        else
        {
        ...
        if (!ecma_is_completion_value_empty (completion_value))
        {
          break;
        }
      }
  ...
  /*
   * Completition value can be an anything including exceptions.
   */
  if (ecma_is_completion_value_empty (completion_value))
  {
    ...
    completion_value = ecma_op_function_call (reviver_p,
