...
{
  if (lit_utf8_iterator_get_pos (&src_iter).offset == 0)
  {
    dump_current_line ();
  }
...
void
lexer_locus_to_line_and_column (lit_utf8_iterator_pos_t locus, size_t *line, size_t *column)
{
  ...
  while (!lit_utf8_iterator_is_eos (&iter) && lit_utf8_iterator_pos_cmp (iter_pos, locus) < 0)
  {
    ...
    if (lit_utf8_iterator_get_pos (&iter).is_non_bmp_middle == false)
    {
      c++;
    }
  }
